...
function normalizeInputPath(inputPath) {
    ...
    // C:\Users\Example\Desktop\a.png -> /mnt/c/Users/Example/Desktop/a.png
    // C:/Users/Example/Desktop/a.png -> /mnt/c/Users/Example/Desktop/a.png
...
function parseArgs(argv) {
    ...
    if (!inputPath) {
        throw new Error(
            [
                "Usage:",
                "  node scripts/convert-store-screenshot.cjs <path-to-screenshot> [--overwrite]",
                "",
                "Examples:",
                "  node scripts/convert-store-screenshot.cjs assets/store/01.png",
                "  node scripts/convert-store-screenshot.cjs assets\\store\\01.png",
                "  node scripts/convert-store-screenshot.cjs 'C:\\Users\\Example\\Desktop\\01.png'",
                "  node scripts/convert-store-screenshot.cjs /mnt/c/Users/example/Desktop/01.png",
                "  node scripts/convert-store-screenshot.cjs assets/store/01.png --overwrite",
            ].join("\n")
        );
    }
...
function createOutputPath(inputPath, { overwrite }) {
    ...
    return path.join(dir, `${base}-store-1280x800.png`);
}
...
function createTemporaryOutputPath(inputPath) {
    ...
    return path.join(
        dir,
        `.${base}.store-1280x800.tmp-${process.pid}-${Date.now()}.png`
    );
}
...
async function convertImage(inputPath, outputPath, { overwrite }) {
    ...
    try {
        ...
        if (
            ...
            throw new Error(
                [
                    "Output is not Chrome Web Store compliant:",
                    `  ${actualOutputPath}`,
...
async function main() {
    ...
    console.log(
        [
            overwrite
                ? "Overwrote Chrome Web Store screenshot:"
                : "Converted Chrome Web Store screenshot:",
            `  input:  ${inputPath}`,
            `          ${describeMetadata(inputMetadata)}`,
            `  output: ${outputPath}`,
            `          ${describeMetadata(outputMetadata)}`,
            "",
            "Result is Chrome Web Store compliant:",
